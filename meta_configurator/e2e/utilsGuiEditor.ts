...
export async function editStringProperty(page: Page, propertyPath: Path, value: string) {
    ...
    const textField = page.getByTestId(`property_data-${pathAsString}`).getByRole('textbox')
...
export async function editBooleanProperty(page: Page, propertyPath: Path, value: boolean) {
    ...
    const button = page.getByTestId(`property_data-${pathAsString}`).getByRole('button', { name: value.toString() });
    await button.click();
}

export async function editNumberOrIntProperty(page: Page, propertyPath: Path, value: number) {
    const pathAsString = pathToString(propertyPath);
    const spinButton = page.getByTestId(`property_data-${pathAsString}`).getByRole('spinbutton')
...
export async function checkNumberOrIntProperty(page: Page, propertyPath: Path, value: number) {
    const pathAsString = pathToString(propertyPath);
    const textField = page.getByTestId(`property_data-${pathAsString}`).getByRole('spinbutton')
...
export async function removeOptionalPropertyValue(page: Page, propertyPath: Path) {
    const pathAsString = pathToString(propertyPath);
    const removeButton = page.getByTestId(`property_data-${pathAsString}`).getByRole('button', { name: 'Remove' });
    await removeButton.click();
}

export async function addObjectProperty(page: Page, propertyPath: Path) {
    const pathAsString = pathToString(propertyPath);
    const addButton = page.getByTestId(`add_property-${pathAsString}`);
    await addButton.click();
}

export async function addArrayItem(page: Page, propertyPath: Path) {
    const pathAsString = pathToString(propertyPath);
    const addButton = page.getByTestId(`add_item-${pathAsString}`);
    await addButton.click();
}

export async function checkPropertySchemaViolation(page: Page, propertyPath: Path, shouldBeVisible: boolean) {
    const pathAsString = pathToString(propertyPath);
    const validationErrorIcon = page.getByTestId(`property_metadata-${pathAsString}`).getByTestId("validation-error-icon");
    if (shouldBeVisible) {
...
export async function checkPropertyRequired(page: Page, propertyPath: Path, shouldBeVisible: boolean) {
    const pathAsString = pathToString(propertyPath);
    const requiredIcon = page.getByTestId(`property_metadata-${pathAsString}`).getByTestId("required-star");
    if (shouldBeVisible) {
...
export async function expandOrCollapseProperty(page: Page, propertyPath: Path) {
    ...
    const expansionButton = page.getByRole('cell', { name: new RegExp(`^${propertyName} :`) }).getByRole('button');

...
function activateSchemaMode() { schemaMode.value = true; }
function activateDataMode() { schemaMode.value = false; }
...
function generateText() {
  ...
  exportService.performDataMapping(document, templateContent.value).then((result: {resultData: any; success: boolean; message: string}) => {
...
async function generateTemplateWithAI() {
  ...
  try {
    ...
    const response = await exportService.generateMappingSuggestion(input, inputSchema, outputDescription);
...
<template>
  <Dialog v-model:visible="showDialog" header="Export to other Formats via Template" :style="{maxWidth: '90%', minWidth: '60%', margin: 'auto'}">
    <div class="flex flex-column gap-3 bigger-dialog-content">


      <Panel
        header="Generate a Template with AI Assistance"
        toggleable
        :collapsed="true">
        ...
        <Textarea v-model="aiDescription" rows="4" class="w-full mt-4" autoResize placeholder="Describe the output file you want..." />
        <Button label="Generate Template" icon="pi pi-robot" class="mt-2" @click="generateTemplateWithAI" :loading="isGeneratingTemplate" />
      </Panel>

      <Textarea v-model="templateContent" rows="10" autoResize class="w-full" placeholder="Write or paste your Handlebars template here..." />
      ...
      <div class="text-sm text-gray-500">
        <a href="https://handlebarsjs.com/guide/" target="_blank" class="underline">Learn more about Handlebars templates</a>
      </div>
    </div>
  </Dialog>
</template>

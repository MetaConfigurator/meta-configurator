...
describe('test schema graph constructor with conditionals', () => {
  ...
  it('generate object title', () => {
    ...
    const thenNode = defs.get('then')!;
    expect(generateObjectTitle(thenNode.absolutePath, thenNode.hasUserDefinedName, thenNode.schema)).toEqual('then');

    const thenNodeObject = defs.get('then.properties.propertyObject')!;
    expect(generateObjectTitle(thenNodeObject.absolutePath, thenNodeObject.hasUserDefinedName, thenNodeObject.schema)).toEqual(
      'propertyObject'
    );
  });

...
function isSchemaMatchingTypeChoice(schema: JsonSchemaObjectType, typeChoiceSchema: JsonSchemaObjectType): boolean {
    if (schema.type !== typeChoiceSchema.type) {
        return false;
    }

    if (schema.type === 'array') {
        return isSchemaMatchingTypeChoice(schema.items as JsonSchemaObjectType, typeChoiceSchema.items as JsonSchemaObjectType);
    }
...
export function applyNewType(currentSchema: JsonSchemaObjectType, typeSchema: JsonSchemaObjectType) {
    currentSchema.type = typeSchema.type
    if (typeSchema.type === 'array') {
        if (currentSchema.items === undefined || currentSchema.items === true || currentSchema.items === false) {

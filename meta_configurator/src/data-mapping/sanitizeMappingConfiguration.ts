...
export function sanitizeMappingConfiguration(config: any, inputFile: any): string {
    ...
    if (configValidationResult.errors.length > 0) {
        const formattedErrors = configValidationResult.errors.map((error) => {
    ...
    if (invalidUsedSourcePaths.length > 0) {
        console.log(`The following source paths are not valid in the input file: ${invalidUsedSourcePaths.join(", ")}. They will be removed from the configuration.`);
    }

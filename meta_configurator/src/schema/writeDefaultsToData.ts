...
export function writeSchemaRequiredDefaultsToData(data: ManagedData, path: Path, schema: JsonSchemaWrapper) {
                    ...
                    if (property.default) {
                        ...
                        if (property.hasType('object')) {
                            ...
                            if (property.items && property.minItems > 0) {
                                writeSchemaRequiredDefaultsToData(data, path.concat(propertyName, '0'), property.items);
                            }
                        }
                    }
                }

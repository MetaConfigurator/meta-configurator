...
describe('test renameUtils', () => {
    ...
    const metaSchemaWrapper = new JsonSchemaWrapper(META_SCHEMA_SIMPLIFIED, SessionMode.SchemaEditor, false);
    ...
    it('test replacePropertyNameUtils on data level', () => {
        ...
        const result = replacePropertyNameUtils(subPath, oldName, newName, data, schemaWrapper, updateDataFct);
    ...
    it('test replacePropertyNameUtils on schema level. This implicitly tests whether references and required props are updated also.', () => {
        ...
        const result = replacePropertyNameUtils(path, oldName, newName, mutableSchema, metaSchemaWrapper, updateDataFct);
        // result is the new path of the renamed property

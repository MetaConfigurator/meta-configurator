...
In this JSON-LD representation, \texttt{element\_size} is modeled as a \href{https://schema.org/PropertyValue}{\texttt{schema:PropertyValue}} resource with a typed numeric value and an explicit unit link to QUDT, just as in RDF/XML. Compared with Listing~\ref{lst:background_rdf_xml_example}, the semantics are equivalent, but the syntax is JSON-based. This makes \gls{jsonld} suitable when one wants RDF-level semantics while keeping compatibility with JSON-oriented tooling and workflows.

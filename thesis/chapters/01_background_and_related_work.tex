...
\subsection{Ontology Engineering and Knowledge Graph Platforms}
...
Table~\ref{tab:related_tools_comparison} compares representative tools across capabilities relevant to this thesis: \gls{rdf} authoring, ontology integration, source-to-\gls{rdf} transformation, \gls{rdf} storage/query, visualization, and \gls{ai} assistance.
...
\end{tabular}
\end{table}
\noindent\textit{Rating scale:}
\begin{itemize}
    \item \dingcheck: native, first-class support.
    \item \textit{Partial}: substantial but scope-limited support (often programmatic rather than end-user focused).
    \item \textit{Limited}: basic or indirect support (e.g., plugin-based, narrow subset, or lightweight/read-only capability).
    \item --: not a core capability.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Gap between structured JSON editing and RDF transformation:} In practice, much scientific data is authored and maintained in hierarchical formats such as \gls{json}. While standards such as \gls{jsonld} provide a bridge to linked data \cite{sporny2014jsonld}, moving from conventional \gls{json} structures to \gls{rdf} still often requires separate mapping artifacts and additional tools (e.g., \gls{r2rml}/\gls{rml}) \cite{das2012r2rml,dimou2014rml}.

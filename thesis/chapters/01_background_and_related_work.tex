...
The increasing adoption of computational experiments and simulation workflows in scientific research has resulted in large volumes of data describing parameters, metrics, and results. In principle, such data can be modeled and validated directly with \gls{rdf}-based standards \cite{rdf_concepts2014,shacl_standard}. In practice, however, many researchers still manage data in ad hoc artifacts such as spreadsheet files, and when machine-readable encodings are used, they are commonly represented in formats such as XML, \gls{json}, or YAML rather than as native \gls{rdf} graphs \cite{metaconfigurator2024,neubauer2026data}. The Semantic Web promises improved interoperability, explicit semantics, and cross-dataset integration \cite{berners2001semantic,janowicz2015geospatial,hogan2021knowledgegraphs}, but its adoption often faces a substantial entry barrier due to ontology engineering effort, identifier and vocabulary design, and mapping complexity \cite{janowicz2015geospatial,hogan2021knowledgegraphs}. Against this background, this chapter introduces one practical workflow, not yet a universal standard: using \gls{json} as the operational data format, JSON Schema for structural validation, and \gls{rml} mappings to generate semantically enriched \gls{jsonld}. The discussion is illustrated using a running example from a simulation experiment where the element size parameter influences a computed stress metric.

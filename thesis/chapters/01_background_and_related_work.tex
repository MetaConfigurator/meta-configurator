...
This enables graph-based queries, reasoning, and inference beyond simple format validation. For example, if an ontology defines that every \texttt{m4i:Method} must \texttt{m4i:hasParameter} only resources of type \texttt{schema:PropertyValue}, then from the statement that \texttt{local:run\_fenics\_simulation} is a \texttt{m4i:Method} and \texttt{m4i:hasParameter} is linked to \texttt{local:variable\_element\_size} node, a reasoner can infer that \texttt{local:variable\_element\_size} is a \texttt{schema:PropertyValue}, even if this type assertion were not explicitly stated.

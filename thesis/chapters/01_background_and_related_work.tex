...
\gls{jsonld} data can be interpreted as \gls{rdf}. For instance, in the simulation example in Listing~\ref{lst:background_rdf_example}, the parameter \texttt{element\_size} is represented as a \href{https://schema.org/PropertyValue}{\texttt{schema:PropertyValue}} resource.

\begin{lstlisting}[float=htbp,style=xml, caption={Representation of \texttt{local:variable\_element\_size} node in \gls{rdf} format}, label={lst:background_rdf_example}]
<rdf:RDF
...
RDF-based Semantic Web technologies thus provide a framework for:
...
By grounding \gls{json} data in ontologies and expressing it as \gls{rdf} through \gls{jsonld}, this simulation result becomes part of the Semantic Web, enabling more advanced analysis, automated reasoning, and integration with other scientific \glspl{kg}.
...
For example, consider the simulation parameter \texttt{element\_size} in the \gls{jsonld} example (Listing~\ref{lst:background_rdf_example}). Using \gls{shacl}, we can define a shape that enforces that every \href{https://schema.org/PropertyValue}{\texttt{schema:PropertyValue}} representing an experiment parameter must have a numeric \href{https://schema.org/value}{\texttt{schema:value}} and a \href{https://schema.org/unitCode}{\texttt{schema:unitCode}} pointing to a valid unit from the \gls{qudt} ontology:

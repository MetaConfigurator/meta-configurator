...
The schema in Listing~\ref{lst:background_json_schema} checks that simulation data has the expected format before it is processed further. It requires every parameter and metric to include a \texttt{name}, a numeric \texttt{value}, and a \texttt{unit}, and it blocks extra unexpected fields (\texttt{additionalProperties: false}). This avoids common problems such as missing units, wrong field names, or wrong data types when files are exchanged between tools. However, JSON Schema only checks structure. It does not describe the domain meaning of terms such as \texttt{element\_size} or \texttt{max\_von\_mises\_stress}. For that reason, we need to incorporate Semantic Web technologies to add explicit meaning and improve interoperability.

...
def looks_like_python_source(content: str) -> bool:
    ...
    return any(
        re.search(pattern, snippet, flags=re.MULTILINE) is not None for pattern in python_signals
    )
...
def _summarize_python_assignment(node: Any, source_bytes: bytes) -> Dict[str, Any]:
    ...
    if target is not None:
        declared_names.append(_compact_tree_text(_tree_sitter_node_text(target, source_bytes)))
    return {
...
def _summarize_python_function(node: Any, source_bytes: bytes, comments: List[str]) -> Dict[str, Any]:
    named_children = list(getattr(node, 'named_children', []))
    name_node = next((child for child in named_children if child.type == 'identifier'), None)
    parameter_node = next((child for child in named_children if child.type == 'parameters'), None)
    ...
    if body is not None:
        for descendant in _iter_named_descendants(body):
            if descendant.type in {'assignment', 'augmented_assignment'}:
                locals_summary.append(_summarize_python_assignment(descendant, source_bytes))
...
def _summarize_python_class(node: Any, source_bytes: bytes, comments: List[str]) -> Dict[str, Any]:
    name = next(
        (
    ...
    block = next((child for child in getattr(node, 'named_children', []) if child.type == 'block'), None)
    ...
    if block is not None:
        for child in getattr(block, 'named_children', []):
            ...
            if child.type == 'function_definition':
                methods.append(_summarize_python_function(child, source_bytes, pending_comments))
                pending_comments = []
                continue
            pending_comments = []
    return {
...
def _summarize_python_module(root: Any, source_bytes: bytes, counts: Dict[str, int]) -> Dict[str, Any]:
    imports: List[str] = []
    ...
    for child in getattr(root, 'named_children', []):
        ...
        if child.type in {'import_statement', 'import_from_statement'}:
            imports.append(_compact_tree_text(_tree_sitter_node_text(child, source_bytes)))
            continue
        if child.type == 'class_definition':
            classes.append(_summarize_python_class(child, source_bytes, pending_comments))
            pending_comments = []
            continue
        if child.type == 'function_definition':
            functions.append(_summarize_python_function(child, source_bytes, pending_comments))
            pending_comments = []
            continue
        if pending_comments:

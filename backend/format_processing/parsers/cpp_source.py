...
def _collect_cpp_top_level_calls(
            ...
            calls.append(
                {
                    ...
                    "arguments": _extract_cpp_argument_values(
                        argument_node, source_bytes
                    ),
                }
            )
            continue
    ...
    if body is not None:
        for descendant in _iter_named_descendants(body):
            if descendant.type == "declaration":
                declarations.append(
                    _summarize_cpp_declaration(descendant, source_bytes)
                )
...
def _summarize_cpp_translation_unit(
    ...
    for child in getattr(root, "named_children", []):
        ...
        if child.type == "function_definition":
            functions.append(
                _summarize_cpp_function(child, source_bytes, pending_comments)
            )
            pending_comments = []
            continue
        benchmark_registration = _summarize_cpp_benchmark_registration(
            child, source_bytes
        )

...
def detect_format_and_parse(
    ...
    message = 'Backend could not recognize a supported format. Falling back to AI mapping.'
    return DetectionResult(
        recognized=False,
        format='unknown',
        parsed_json=None,
...
def preprocess_parsed_data_for_ai(
    ...
    return {
        ...
        'ai_prompt_hint': AI_PROMPT_HINTS.get(format_name, AI_PROMPT_HINTS.get('json', '')),
    }
...
def _get_prioritized_parsers(
    ...
    if re.search(r'^\s*(export\s+)?[A-Za-z_][A-Za-z0-9_]*\s*=\s*.+$', content, flags=re.MULTILINE):

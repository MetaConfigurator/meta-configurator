...
class TestFormatDetection(unittest.TestCase):
        ...
        for file_name, mime_type, expected_format in cases:
            with self.subTest(file_name=file_name):
                if expected_format == 'yaml' and fd.yaml is None:
                    self.skipTest('Skipping YAML detection test because PyYAML is not installed.')
                if expected_format == 'xml' and fd.xmltodict is None:
                    self.skipTest('Skipping XML detection test because xmltodict is not installed.')
                ...
                if expected_format == 'ttl':
                    ...
                    self.assertTrue(
                        'triples' in result.parsed_json or 'raw_ttl' in result.parsed_json
                    )
                ...
                if expected_format == 'cpp_source':
                    ...
                    self.assertEqual(result.parsed_json.get('root_type'), 'translation_unit')
                    self.assertEqual(result.parsed_json.get('representation'), 'syntax_tree_with_summary')
                    ...
                    self.assertTrue(
                        any(
                            isinstance(function, dict) and function.get('name') == 'main'
                ...
                if expected_format == 'python_source':
                    ...
                    self.assertEqual(result.parsed_json.get('representation'), 'summary')
                    ...
                    self.assertTrue(
                        any(
                            isinstance(function, dict) and function.get('name') == 'build'
    ...
    def test_star_when_gemmi_available(self) -> None:
        if fd.gemmi is None:
            self.skipTest('Skipping STAR detection test because gemmi is not installed.')
        ...
        self.assertIsInstance(result.parsed_json['sample_001']['_atom_site_label'], list)
        self.assertGreaterEqual(len(result.parsed_json['sample_001']['_atom_site_label']), 3)
...
""".strip()
        ...
        self.assertIsInstance(result.parsed_json, dict)
        self.assertIn('syntax_tree', result.parsed_json)
        registrations = result.parsed_json.get('summary', {}).get('benchmark_registrations', [])
    ...
    def test_preprocess_for_ai_accepts_custom_options(self) -> None:
        ...
        self.assertLess(len(strict_preprocessed['rows']), len(default_preprocessed['rows']))

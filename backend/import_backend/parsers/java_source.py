...
def looks_like_java_source(content: str) -> bool:
    ...
    return any(
        re.search(pattern, snippet, flags=re.MULTILINE) is not None for pattern in java_signals
    )
...
def _extract_java_declared_names(node: Any, source_bytes: bytes) -> List[str]:
    ...
    for descendant in _iter_named_descendants(node):
        if descendant.type == 'variable_declarator':
            identifier = next(
                (child for child in getattr(descendant, 'named_children', []) if child.type == 'identifier'),
                None,
            )
...
def _collect_java_calls(node: Any, source_bytes: bytes) -> List[Dict[str, Any]]:
    ...
    for descendant in _iter_named_descendants(node):
        if descendant.type not in {'method_invocation', 'object_creation_expression'}:
            continue
        argument_node = next(
            (child for child in getattr(descendant, 'named_children', []) if child.type == 'argument_list'),
            None,
        )
        arguments = []
...
def _summarize_java_method(node: Any, source_bytes: bytes, comments: List[str]) -> Dict[str, Any]:
    named_children = list(getattr(node, 'named_children', []))
    name_node = next((child for child in named_children if child.type == 'identifier'), None)
    parameter_node = next((child for child in named_children if child.type == 'formal_parameters'), None)
    ...
    if body is not None:
        for descendant in _iter_named_descendants(body):
            if descendant.type == 'local_variable_declaration':
                locals_summary.append(_summarize_java_declaration(descendant, source_bytes))
            elif descendant.type in {'for_statement', 'enhanced_for_statement', 'while_statement', 'do_statement'}:
                loops.append(
                    {
                        'line': descendant.start_point.row + 1,
            ...
            elif descendant.type in {'if_statement', 'switch_expression', 'switch_statement'}:
                conditionals.append(
                    {
                        'line': descendant.start_point.row + 1,
                        'type': descendant.type,
    ...
    return {
        'line': node.start_point.row + 1,
        'name': _tree_sitter_node_text(name_node, source_bytes) if name_node is not None else None,
        'signature': _compact_tree_text(_tree_sitter_node_text(node, source_bytes).split('{', 1)[0]),
...
def _summarize_java_class(node: Any, source_bytes: bytes, comments: List[str]) -> Dict[str, Any]:
    name = next(
        (
            _tree_sitter_node_text(child, source_bytes)
            for child in getattr(node, 'named_children', [])
    ...
    body = next((child for child in getattr(node, 'named_children', []) if child.type == 'class_body'), None)
    methods: List[Dict[str, Any]] = []
    pending_comments: List[str] = []
    if body is not None:
        for child in getattr(body, 'named_children', []):
            if child.type == 'comment':
                pending_comments.append(_normalize_comment_text(_tree_sitter_node_text(child, source_bytes)))
                continue
            if child.type in {'method_declaration', 'constructor_declaration'}:
                methods.append(_summarize_java_method(child, source_bytes, pending_comments))
                pending_comments = []
                continue
            pending_comments = []
    return {
...
def _summarize_java_program(root: Any, source_bytes: bytes, counts: Dict[str, int]) -> Dict[str, Any]:

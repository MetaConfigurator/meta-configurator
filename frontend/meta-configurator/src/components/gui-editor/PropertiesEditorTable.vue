...
<template>
  <table class="w-full table-fixed p-1">
    ...
    <tbody v-for="(schemaDef, key) in schemaAtCurrentPath.properties" :key="key">
      <tr>
        <td class="p-2 bg-slate-200 border-b border-r border-slate-300 flex flex-row items-center">
          ...
          {{ key }}
          <span v-if="isRequired(key as string)" class="text-red-500">*</span>
          <span class="ml-2 text-xs text-gray-400">{{ schemaDef.type }}</span>
          <IconExpand

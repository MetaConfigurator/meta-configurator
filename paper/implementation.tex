...
This part shows how the GUI representations of basic JSON Schema features look like. The table \ref{table:json_schema_type_component} summarises the basic json schema features and also some complex features and how they are represented in the GUI.
...
\begin{table*}
    ...
    \begin{tabular}{@{}p{0.2\linewidth}p{0.6\linewidth}@{}}
        ...
        boolean & Checkbox to switch between true, false or unchosen options.\\
        array & List of text fields, which is collapsed by default.\\
        object & Table for the key value pairs. Properties are shown when table is expanded \\
        enum(enumeration) & Dropdown box for different options.\\
        const(constant) & Non-editable text field. It's implemented by enumeration with only one entry\\
        required & Simple red star near property name indicates the property is required.\\
        deprecated & Grey line across property name. This indicates that the property should not be used and may be removed in the future.\\

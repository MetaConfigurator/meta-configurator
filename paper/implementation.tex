...
In order to ensure that our data is valid as soon as it's received. We need to have our config data get validated based on our schema. So we choose to use a useful tool named \textbf{Ajv JSON schema validator} \cite{ajv-validator}. Ajv generates code to turn JSON Schemas into super-fast validation functions. It could naturally support the newest JSON schema draft 2020-12. In the code editor, if config data is violated against the schema, there will be a small error hint on left side, which is very helpful for users.

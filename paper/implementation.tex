...
However, applying our generic approach on the offical Json schema meta schema (TODO citation) does not result in a user-friendly editor for
...
With the \tt{description} keyword, schema authors can give descriptions to any elements of their schema.
...
Also, \toolname does not support the \tt{\$vocabulary} keyword yet.
Both features are used in the JSON schema meta schema, as it is distributed over multiple schema files.
To circumvent that problem, we put all schemas in one schema file into the \tt{\$defs} object and replace the external references with local references.
...
The difference of those keywords in comparison to the \tt{\$ref} keyword is that they provide a way to dynamically extent the JSON meta schema
at runtime.
...
We replaced all of them by "non-dynamic" references using the \tt{\$ref} keyword.

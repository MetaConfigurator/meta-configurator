...
\begin{figure}[h]
    ...
    \caption{GUI Panel}
\end{figure}
each serves a unique purpose and offers a different set of functionalities.
...
The Schema Editor view allows users to define and manage schema templates for configuration files. This view is essential for maintaining consistency and structure within your configurations. Details of the Schema Editor and its functionalities can be inserted here.
...
The Settings view provides customization options for tailoring the tool to your specific needs. It allows user to configure various preferences and settings to enhance user's workflow. Details of the Settings view and its functionalities can be inserted here.
\begin{figure}[h]
...
The Settings view also includes a GUI editor panel, which allows users to configure various settings:
\begin{figure}[h]
    \includegraphics[width=3.5 in]{figures/GUI Editor Pannel Settings}
    \caption{Settings's GUI Pannel}
\end{figure}
\begin{itemize}
    \item \textbf{Data Format:} Users can select between JSON and YAML data formats. The choice made here will influence the schema and configuration format used in both the Schema Editor and File Editor.
...
\begin{enumerate}
    ...
    \item Upon initial access, a default page is displayed, and users must select their desired Schema. We have implemented four options for this selection:
    \begin{enumerate}
        \item From File

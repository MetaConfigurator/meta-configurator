...
\paragraph{Convert types to oneOf}
...
An exception is when a type can not be merged with a oneOf sub-schema (e.g. the type is "boolean" and the oneOf sub-schema has type "string").
...
\paragraph{Remove incompatible oneOfs}
It can happen that the previous pre-processing steps produce oneOf options which are not compatible with the schema of the property (e.g. sub-schemas that can never be fulfilled in combination with the property schema). Listing \ref{listing:incompatible_one_of_producer} shows a schema that will produce such incompatible oneOf options. 
        ...
        tabsize=4]{js}
{
  ...
  "properties": {
    ...
    \caption{Simple JSON schema that will produce incompatible oneOf options after conversion of types to oneOf}
    \label{listing:incompatible_one_of_producer}
\end{listing}
        ...
        tabsize=4]{js}
{
  ...
  "properties": {
    ...
    \caption{Simple JSON schema with incompatible oneOf options after conversion of types to oneOf}
    \label{listing:incompatible_one_of_producer}
\end{listing}
